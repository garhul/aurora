...
bool init(const char* broker, const char* topic,
  ...
  while (!client.connect(Settings::getDeviceId().c_str()) &&
         attempts < CONN_RETRIES) {
    Serial.print(".");
    attempts++;
    delay(1000);
  }
...
void announce() {
  ...
  Serial.println("Announcing on " + String(Settings::settings.announce_topic) +
                 "   " + Settings::getAnnounceInfo());

  client.publish(Settings::settings.announce_topic,
                 Settings::getAnnounceInfo());
}
