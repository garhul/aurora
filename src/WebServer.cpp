...
namespace WebServer {
  ...
  void init(void (*f)(String, String)) {
    ...
    server.on("/clear", HTTP_POST,
              _clearCredentials);  // endpoint for clearing ssid / pwd
  ...
  void _cmd() {
    ...
    if (!server.hasArg("cmd")) {
      server.send(400, "application/json",
                  "{\"message\":\"Missing command[cmd] parameter\"}");
      return;
    }
  ...
  void _setup() {
    ...
    if (!server.hasArg("pass") || !server.hasArg("ssid") ||
        !server.hasArg("topic") || !server.hasArg("broker") ||
        !server.hasArg("announce_topic") || !server.hasArg("human_name") ||
        !server.hasArg("ap_ssid") || !server.hasArg("strip_size")) {
      server.send(
          400, "text/plain",
          "Invalid params one of [pass, ssid, topic, broker, "
          "announce_topic, human_name, ap_ssid, strip_size] is missing");
      return;
    }
  ...
  void _info() {
    server.send(200, "application/json", Settings::getInfoJson());
  }
